// GET /oauth/google — recebe callback OAuth do Google e repassa ao dashboard via postMessage
export default function handler(req, res) {
  ...
  res.status(200).send(oauthPage(success, code, error, origin, 'google_oauth_callback'));
}

function oauthPage(success, code, error, origin, type) {
  const emoji  = success ? '✅' : '❌';
  ...
  try{
    if(window.opener){
      window.opener.postMessage(
        {type:${JSON.stringify(type)},code:${JSON.stringify(code)},error:${JSON.stringify(error)}},
